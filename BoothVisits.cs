...
namespace TPO
{
    public partial class BoothVisits : Form
    {
        ...
        private void BoothVisits_Load(object sender, EventArgs e)
        {
            // Clear existing columns if any
            dataGridView1.Columns.Clear();

            // Manually add columns (only headers, no data)
            /*dataGridView1.Columns.Add("BoothID", "Booth ID");
             dataGridView1.Columns.Add("check_intime", "Check-in Time");
             dataGridView1.Columns.Add("UserId", "User ID");
             dataGridView1.Columns.Add("CompanyID", "Company ID");
        ...
        private void LoadSkills(SqlConnection conn)
        {
            try
            {
                ...
                FROM Student S
                JOIN Visits v ON S.StudentId = v.Userid
